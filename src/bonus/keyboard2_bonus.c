...
/*   Updated: 2024/06/27 12:54:40 by incalero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_move(t_data *data, double cosValue, double sinValue)
{
	...
	if (data->player.pos_y != -1 && data->player.pos_x != -1)
	{
		...
		COLISION_DETECTOR || data->player.pos_y < 300 + \
		COLISION_DETECTOR || data->player.pos_x > \
		(data->cols - 1) * 300 - COLISION_DETECTOR || data->player.pos_y > \
		(data->rows - 1) * 300 - COLISION_DETECTOR)
		{
...
void	make_move3(t_data *data)
{
	...
	if (data->player.pos_y != -1 && data->player.pos_x != -1)
	{
		...
		COLISION_DETECTOR || data->player.pos_y < 300 + \
		COLISION_DETECTOR || data->player.pos_x > \
		(data->cols - 1) * 300 - COLISION_DETECTOR || \
		data->player.pos_y > (data->rows) * 300 - COLISION_DETECTOR)
		{
			data->player.pos_x = data->player.previous_pos_x;
			data->player.pos_y = data->player.previous_pos_y;
		}
